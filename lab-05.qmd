...
> The 'zero_q_freq' is the frequency of days where flow (q) = 0 mm/day, with percentage as the unit of measurement.
...
```{r}
p1 <- ggplot(camels) +
  annotation_borders("state", colour = "grey7", fill = "grey") +
  geom_point(aes(x = gauge_lon, y = gauge_lat, color = aridity)) +
  scale_color_viridis_c(name = "Aridity", 
                        option = "plasma", 
                        direction = 1, 
                        begin = 0.3, 
                        end = 1) +
...
p2 <- ggplot(camels) +
  annotation_borders("state", colour = "grey7", fill = "grey") +
  geom_point(aes(x = gauge_lon, y = gauge_lat, color = p_mean)) +
  scale_color_viridis_c(name = "Mean Daily Precipitation", 
                        option = "turbo", 
                        direction = -1) +
  labs(color = "Mean Precipitation") +
  theme_void() 

(p1 | p2) + plot_annotation(title = "Camel Sites: Aridity and Precipitation",
                            caption = "Figure 1: Aridity index (left) and mean daily precipitation (right) for CONUS camel sites.") &
...
```{r}
# add xgboost and nnet models to workflow
wf_add <- workflow_set(list(rec), list(lm_model, rf_model, xgb_model, nnet_model)) |>
  workflow_map('fit_resamples', resamples = camels_cv) 

autoplot(wf_add)

ranks <- rank_results(wf_add, rank_metric = "rsq", select_best = TRUE)

kableExtra::kable(ranks)
```

> Out of the four models, the Neural Network (bag_mlp) model ranks highest in both RMSE and RSQ. The Random Forest (rand_forest) and linear regression (linear_reg) models perform slightly worse than the Neural Network model, but are comparable to eachother in performance. The Boosted Tree (boost_tree) model performs the poorest, with a larger degradation in metrics from other models.

> In the case of camels data, I would move forward with the Random Forest model. Even though the Neural Network outperforms the other models, they are harder to interpret as much of their computations happen in a "black box". The Random Forest model trades a slight performance decrease for increase in tuning and interprebility.
...
camels_vars <- camels |> 
  select("soil_conductivity", "max_water_content", "geol_1st_class", "geol_porostiy", "geol_permeability",
         "frac_forest", "gvf_diff", "dom_land_cover", "runoff_ratio", "stream_elas", "slope_fdc", 
         "baseflow_index", "q5", "q95", "high_q_freq", "low_q_freq", "zero_q_freq", "p_mean", "pet_mean", 
         "p_seasonality", "frac_snow", "high_prec_freq",  "low_prec_freq", "low_prec_timing", "aridity", 
         "elev_mean", "slope_mean", "area_gages2","gauge_lat", "gauge_lon", "q_mean", "gauge_id") |> 
  drop_na() |>
  mutate(logQmean = log(q_mean)) |>
  select(-q_mean)
```
...
camels_vars |> select(where(is.numeric)) |> 
  vis_cor()
...
```{r}
#| label: spatial-autocorrelation-check

coords <- camels_vars |>
  select(gauge_lon, gauge_lat) |>
  as.matrix()

neighbors <- knearneigh(coords, k = 5, longlat = TRUE) |>
  knn2nb()
...
```{r}
#| label: spatial-autocorrelation-moran

morans_i <- camels_vars |>
  select(where(is.numeric), -gauge_lon, -gauge_lat) |>
  map_dfr(\(x) {
    res <- moran.test(x, weights, na.action = na.omit)
    tibble(morans_I = res$estimate[[1]], p_value = res$p.value)
  }, .id = "variable") |>
...
```{r}
#| label: spatial-autocorrelation-shapiro

shapiro_results <- camels_vars |>
  drop_na() |>
  select(where(is.numeric)) |>
  map_dfr(\(x) tidy(shapiro.test(x)), .id = "variable") |>
...
camels_split <- initial_split(camels_vars, prop = 0.75, strata = aridity_class)

camels_train <- training(camels_split)
camels_test <- testing(camels_split)
```

> Data continuity seems high, with little missingness throughout most vaariables. The variable 'geol_2nd_class' is the only variable with significant missingness with 79.4% completeness. I will remove this variable from the dataset and impute means for all other NAs. The feature distribution is varied with many left and right skewed distributions as well as some that are approximately normal and several uniform distributions. All features will need to be scaled and normalized.
...
```{r}
#| label: recipie-formula

recipe <- recipe(logQmean ~ ., data = camels_train) |>
  # remove coords
  step_rm(gauge_lon, gauge_lat, aridity_class) |>
  update_role(gauge_id, new_role = "ID") |> 
  # fill in NA numerics with imputed means
  step_impute_mean(all_numeric_predictors()) |>
  step_novel(all_factor_predictors()) |> 
  step_dummy(all_factor_predictors()) |>
  #standardize all numeric factors
  step_normalize(all_numeric_predictors())

prepped_data <- prep(recipe, training = camels_train)

baked_data <- bake(prepped_data, new_data = NULL)
```

> Exploratory data analysis showed that several predictors were heavily right-skewed, resulting in the use of a log transformation. Missing values were addressed with mean imputation. All predictors were normalized to ensure a common scale and prevent domination by variables with larger magnitudes or differing units. The interaction between aridity and rainfall was added to capture the non-linear relationship between these predictors and mean streamflow. The coordinates (gauge_lon and gauge_lat) were removed from the recipe to prevent the model from learning spatial proximity as a proxy for mean streamflow, which would lead to overfitting and poor generalization to new locations.
...
> Random forests are a strong baseline for regression problems, especially when there are non-linear relationships and interactions between predictors. They can capture complex patterns in the data without requiring extensive tuning.

```{r}
#| label: xgb-tree-model
...
> Boosted trees are an ensemble method that build sequential trees to correct the errors of previous trees. They can capture complex relationships and often achieve high predictive performance with tabular data, making them a good candidate for this regression problem.
...
> Decision trees provide a simple and interpretable model that can capture non-linear relationships and interactions between predictors.
...
wf_random_cv <- workflow_set(list(recipe), 
                            list(rf_model, 
                                 xgb_model, 
                                 dt_model)) |> 
  workflow_map("fit_resamples", resamples = normal_cv)

wf_obj <- workflow_set(list(recipe), 
                       list(rf_model, 
                            xgb_model, 
                            dt_model)) |> 
...
autoplot(wf_random_cv) / autoplot(wf_obj)

rank_results(wf_random_cv, 
             rank_metric = "rsq", 
             select_best = TRUE)
...
> The boosted tree model performs best under both random and spatial CV, followed by the random forest and then the decision tree. The performance drop from random to spatial CV is around 0.05 for the boosted tree and random forest models, while the decision tree experiences a larger degradation. I trust the spatial CV estimate as a more honest generalization estimate, as it accounts for the spatial structure in the data. The spread of per-fold metrics indicates that the models struggle more with certain spatial blocks, suggesting that there may be regional differences in the relationships between predictors and the outcome variable.
...
xgb_wf<- workflow(recipe, xgb_tuned)
```
...
auto_browse(xgb_grid_tuned)

show_best(xgb_grid_tuned, 
          metric = "rsq")

top_preds <- select_best(xgb_grid_tuned,
                         metric = "rsq")

top_preds
```

> Grid space filling allows us to construct parameter grids that try to cover the parameter space such that any portion the space has does not have an observed combination that is unnecessarily close to any other point.

> The best hyperparameter combination for the xgboost model is mtry = 14 and min_n = 4. The tuning landscape shows that the model's performance is sensitive to changes in both hyperparameters, with certain combinations leading to significantly better R-squared values.

```{r}
#| label: xgb-final-wf

final_wf <- xgb_wf %>%
  finalize_workflow(top_preds)
...
extract_fit_parsnip(final_fit) %>% 
  vip(num_features = 10) +
  labs(title = "XGBoost: Top Predictors of Mean Q")
```

> The top predictors of most importance in the XGBoost model are q95 and p_mean. Both of these predictors have high Moran's I values, indicating that they are highly spatially autocorrelated. This suggests that their importance in the model may be due to their ability to act as geographic proxies, capturing location-specific information that is relevant for predicting mean streamflow. The importance of these predictors likely reflects a combination of both mechanism and location.
...
```{r}
#| label: finalized-workflow-metrics
...
final_preds %>% 
  ggplot(aes(x = .pred, y = logQmean, color = aridity_class))+
  geom_point()+
