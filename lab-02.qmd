...
zone_cities_4326 <- st_transform(cities_in_zone, 4326)

# create color palettes and helper func
...
make_popup <- function(d) {
  paste0("<strong>", d$city, ", ", d$state_id, "</strong><br>",
         ...
         "Distance to Mexico: ", round(d$dist_to_mexico, 1), " km<br>",
         "Distace to Canada: ", round(d$dist_to_canada, 1), " km") 
  }
...
leaflet() %>%
  ...
  addLegend(colors = c("seagreen", "darkred"), labels = c("Equidistant", "Border Zone"), 
...
> In order to convert 'eqdc'to 'leaflet'-compatible coordinates, st_transform() was used to reproject the data to ESPG:4326, the standard web mapping CRS. To keep the map responsive, the "all cities" layer is clustered until zoom level 6, at which the individual city markers are shown. This allows the map to load quickly at the national scale while providing individual details when zoomed to a regional level.
---
